...
(function (root) {
  ...
  // Tiếp điểm từ 1 điểm P nằm NGOÀI đường tròn (O, r) -> mảng 0 hoặc 2 điểm (thứ tự cố định,
  // dùng pickSolution/pick 0|1 giống intersectCircles). P nằm trong/trên đường tròn -> [] (không
  // dựng được tiếp tuyến thật sự, để caller đẩy lỗi rõ ràng thay vì trả về giá trị suy biến).
  function tangentPoints(P, O, r) {
    var d = distance(P, O);
    if (d < r + EPSILON) return [];
    var theta = Math.atan2(P.y - O.y, P.x - O.x);
    var alpha = Math.acos(Math.max(-1, Math.min(1, r / d)));
    return [
      pt(O.x + r * Math.cos(theta + alpha), O.y + r * Math.sin(theta + alpha)),
      pt(O.x + r * Math.cos(theta - alpha), O.y + r * Math.sin(theta - alpha))
    ];
  }
  ...
  function resolvePointOp(node, points, errors) {
    ...
    try {
      switch (node.op) {
        ...
        case 'tangentPoint': {
          var tgP = points[node.from];
          var tgCirc = resolveCircleRef(node.circle, points);
          if (!isValidPoint(tgP) || !tgCirc) return undefined;
          var tgSols = tangentPoints(tgP, tgCirc.center, tgCirc.r);
          if (!tgSols.length) { errors.push(id + ': điểm ' + node.from + ' nằm trong hoặc trên đường tròn, không dựng được tiếp tuyến.'); return null; }
          return pickSolution(tgSols, node, points);
        }
        case 'pointOnLine': {
  ...
  // Gom mọi id điểm mà 1 mark tham chiếu tới, tuỳ theo type (dùng để kiểm tra tồn tại trước khi vẽ).
  function markPointIds(m) {
    var ids = [];
    if (m.at) ids.push(m.at);
    if (Array.isArray(m.rays)) ids = ids.concat(m.rays);
    if (Array.isArray(m.segment)) ids = ids.concat(m.segment);
    if (Array.isArray(m.segments)) m.segments.forEach(function (s) { ids = ids.concat(s); });
    return ids;
  }
  ...
  function sceneFromProgram(spec) {
    ...
    // "arcs": chỉ vẽ 1 cung (center/from/to phải là id điểm đã dựng/khai báo hợp lệ).
    (spec.arcs || []).forEach(function (a) {
      if (!a || !a.center || !a.from || !a.to) return;
      if (![a.center, a.from, a.to].every(function (id) { return isValidPoint(scene.points[id]); })) {
        if (typeof console !== 'undefined' && console.warn) console.warn('[Geo2D] arc bị bỏ qua do tham chiếu điểm không hợp lệ:', a);
        return;
      }
      scene.arcs.push({ center: a.center, from: a.from, to: a.to, dashed: !!a.dashed, major: !!a.major });
    });
    // "vectors": đoạn có mũi tên ở đầu "to".
    (spec.vectors || []).forEach(function (v) {
      if (!v || !v.from || !v.to) return;
      if (![v.from, v.to].every(function (id) { return isValidPoint(scene.points[id]); })) {
        if (typeof console !== 'undefined' && console.warn) console.warn('[Geo2D] vector bị bỏ qua do tham chiếu điểm không hợp lệ:', v);
        return;
      }
      scene.vectors.push({ from: v.from, to: v.to, dashed: !!v.dashed });
    });
    // "marks": ký hiệu trang trí (góc vuông, cung góc, tick bằng nhau, độ dài). Mark nào tham
    // chiếu điểm không tồn tại/không hợp lệ thì bỏ qua âm thầm (chỉ log warn) — không làm vỡ hình,
    // vì đây là trang trí chứ không phải hình học lõi.
    (spec.marks || []).forEach(function (m) {
      if (!m || !m.type) return;
      var ids = markPointIds(m);
      if (!ids.length || !ids.every(function (id) { return isValidPoint(scene.points[id]); })) {
        if (typeof console !== 'undefined' && console.warn) console.warn('[Geo2D] mark bị bỏ qua do tham chiếu điểm không hợp lệ:', m);
        return;
      }
      scene.marks.push(m);
    });
  ...
  function sceneFromLegacy(spec) {
    ...
    var scene = { points: {}, labels: {}, segments: [], auxSegments: [], circles: [], polygons: [], arcs: [], vectors: [], marks: [], pointsOrder: [], errors: [] };
    var counter = 0;
  ...
  // Vector đơn vị (px) từ (x,y) — an toàn khi độ dài ~0.
  function normPx(x, y) {
  ...
  // Path SVG cho 1 cung tròn tâm (cx,cy) bán kính r (toạ độ PIXEL, đã qua sx/sy) từ (ax,ay) tới
  // (bx,by). major=false -> vẽ cung nhỏ (<=180°) giữa 2 điểm; major=true -> vẽ cung phản (>180°).
  // Vì computeLayout dùng chung 1 scale cho cả x/y nên đường tròn vẫn tròn trong không gian pixel,
  // tính góc trực tiếp ở đây là đủ chính xác (không cần quy đổi elip).
  function arcSvgPath(cx, cy, r, ax, ay, bx, by, major) {
    var angA = Math.atan2(ay - cy, ax - cx);
    var angB = Math.atan2(by - cy, bx - cx);
    var diff = normalizeAngle2PI(angB - angA); // [0, 2π)
    var minorSweep = diff <= Math.PI ? 1 : 0;
    var sweep = major ? (1 - minorSweep) : minorSweep;
    var largeArc = major ? 1 : 0;
    return 'M ' + ax.toFixed(1) + ',' + ay.toFixed(1) + ' A ' + r.toFixed(1) + ',' + r.toFixed(1) +
      ' 0 ' + largeArc + ' ' + sweep + ' ' + bx.toFixed(1) + ',' + by.toFixed(1);
  }

  /* ---------------------------------- 6b. MARKS (ký hiệu hình học) ------------------------------ */
  /* Trang trí thuần tuý (không ảnh hưởng bố cục/nhãn) — vẽ trên lớp riêng, sau hình học chính,
   * trước điểm/nhãn. Mỗi hàm tự bỏ qua (trả về '') nếu điểm tham chiếu không hợp lệ. */

  function renderRightAngleMark(scene, layout, m, MAIN) {
    var B = scene.points[m.at], A = scene.points[m.rays[0]], C = scene.points[m.rays[1]];
    if (![B, A, C].every(isValidPoint)) return '';
    var sx = layout.sx, sy = layout.sy;
    var bx = sx(B.x), by = sy(B.y);
    var d1 = normPx(sx(A.x) - bx, sy(A.y) - by);
    var d2 = normPx(sx(C.x) - bx, sy(C.y) - by);
    var s = 14;
    var p1x = bx + d1.x * s, p1y = by + d1.y * s;
    var p2x = p1x + d2.x * s, p2y = p1y + d2.y * s;
    var p3x = bx + d2.x * s, p3y = by + d2.y * s;
    return '<path d="M ' + p1x.toFixed(1) + ',' + p1y.toFixed(1) + ' L ' + p2x.toFixed(1) + ',' + p2y.toFixed(1) +
      ' L ' + p3x.toFixed(1) + ',' + p3y.toFixed(1) + '" fill="none" stroke="' + MAIN + '" stroke-width="1.8"/>';
  }

  function renderAngleArcMark(scene, layout, m, MAIN) {
    var B = scene.points[m.at], A = scene.points[m.rays[0]], C = scene.points[m.rays[1]];
    if (![B, A, C].every(isValidPoint)) return '';
    var sx = layout.sx, sy = layout.sy;
    var bx = sx(B.x), by = sy(B.y);
    var dA = normPx(sx(A.x) - bx, sy(A.y) - by);
    var dC = normPx(sx(C.x) - bx, sy(C.y) - by);
    var count = Math.max(1, Math.min(3, +m.count || 1));
    var baseR = 20, step = 6, lastR = baseR;
    var svg = '';
    for (var i = 0; i < count; i++) {
      var r = baseR + i * step;
      lastR = r;
      var ax = bx + dA.x * r, ay = by + dA.y * r;
      var cx = bx + dC.x * r, cy = by + dC.y * r;
      svg += '<path d="' + arcSvgPath(bx, by, r, ax, ay, cx, cy, false) + '" fill="none" stroke="' + MAIN + '" stroke-width="1.6"/>';
    }
    if (m.label) {
      var midDir = normPx(dA.x + dC.x, dA.y + dC.y);
      var lx = bx + midDir.x * (lastR + 14), ly = by + midDir.y * (lastR + 14);
      svg += '<text x="' + lx.toFixed(1) + '" y="' + ly.toFixed(1) + '" font-size="14" font-family="Inter,sans-serif" fill="' + MAIN + '" text-anchor="middle" dominant-baseline="middle">' + esc(m.label) + '</text>';
    }
    return svg;
  }

  function renderEqualTicksMark(scene, layout, m, MAIN) {
    var sx = layout.sx, sy = layout.sy;
    var count = Math.max(1, Math.min(3, +m.count || 1));
    var svg = '';
    (m.segments || []).forEach(function (segPair) {
      var A = scene.points[segPair[0]], B = scene.points[segPair[1]];
      ...
      for (var i = 0; i < count; i++) {
        ...
        var cxp = mx + ux * off, cyp = my + uy * off;
        var x1 = cxp - px * tickLen, y1 = cyp - py * tickLen, x2 = cxp + px * tickLen, y2 = cyp + py * tickLen;
        svg += '<line x1="' + x1.toFixed(1) + '" y1="' + y1.toFixed(1) + '" x2="' + x2.toFixed(1) + '" y2="' + y2.toFixed(1) + '" stroke="' + MAIN + '" stroke-width="1.8" stroke-linecap="round"/>';
      }
    });
    return svg;
  }

  function renderLengthMark(scene, layout, m, MAIN) {
    var sx = layout.sx, sy = layout.sy;
    var seg = m.segment;
    if (!seg) return '';
    var A = scene.points[seg[0]], B = scene.points[seg[1]];
    if (!isValidPoint(A) || !isValidPoint(B)) return '';
    ...
    var mx = (ax + bx) / 2, my = (ay + by) / 2;
    var offset = 16;
    var lx = mx + px * offset, ly = my + py * offset;
    return '<text x="' + lx.toFixed(1) + '" y="' + ly.toFixed(1) + '" font-size="15" font-family="Inter,sans-serif" fill="' + MAIN + '" font-weight="600" text-anchor="middle" dominant-baseline="middle" paint-order="stroke" stroke="var(--paper-3)" stroke-width="4" stroke-linejoin="round">' + esc(text) + '</text>';
  }

  function renderMarksSVG(scene, layout, MAIN) {
    var svg = '';
    (scene.marks || []).forEach(function (m) {
      if (m.type === 'rightAngle') svg += renderRightAngleMark(scene, layout, m, MAIN);
      else if (m.type === 'angleArc') svg += renderAngleArcMark(scene, layout, m, MAIN);
      else if (m.type === 'equalTicks') svg += renderEqualTicksMark(scene, layout, m, MAIN);
      else if (m.type === 'length') svg += renderLengthMark(scene, layout, m, MAIN);
    });
    return svg;
  }
  ...
  function renderSceneSVG(scene, opts) {
    ...
    if (scene.vectors && scene.vectors.length) {
      svg += '<defs>' +
        '<marker id="geo2d-arrow-main" viewBox="0 0 10 10" refX="8" refY="5" markerWidth="7" markerHeight="7" orient="auto-start-reverse"><path d="M0,0 L10,5 L0,10 z" fill="' + MAIN + '"/></marker>' +
        '<marker id="geo2d-arrow-aux" viewBox="0 0 10 10" refX="8" refY="5" markerWidth="7" markerHeight="7" orient="auto-start-reverse"><path d="M0,0 L10,5 L0,10 z" fill="' + AUX + '"/></marker>' +
        '</defs>';
    }
    ...
    // Layer 1.5: arcs (chỉ 1 cung, không phải cả đường tròn)
    (scene.arcs || []).forEach(function (a) {
      var c = scene.points[a.center], A = scene.points[a.from], B = scene.points[a.to];
      if (!isValidPoint(c) || !isValidPoint(A) || !isValidPoint(B)) return;
      var r = distance(c, A);
      if (r < EPSILON) return;
      var stroke = a.dashed ? AUX : MAIN;
      var dashAttr = a.dashed ? ' stroke-dasharray="7,6"' : '';
      svg += '<path d="' + arcSvgPath(sx(c.x), sy(c.y), r * layout.scale, sx(A.x), sy(A.y), sx(B.x), sy(B.y), a.major) +
        '" fill="none" stroke="' + stroke + '" stroke-width="2.4" stroke-linecap="round"' + dashAttr + '/>';
    });
    ...
    // Layer 3.5: vectors (đoạn có mũi tên)
    (scene.vectors || []).forEach(function (v) {
      var a = scene.points[v.from], b = scene.points[v.to];
      if (!isValidPoint(a) || !isValidPoint(b)) return;
      var stroke = v.dashed ? AUX : MAIN;
      var markerId = v.dashed ? 'geo2d-arrow-aux' : 'geo2d-arrow-main';
      var dashAttr = v.dashed ? ' stroke-dasharray="7,6"' : '';
      svg += '<line x1="' + sx(a.x).toFixed(1) + '" y1="' + sy(a.y).toFixed(1) + '" x2="' + sx(b.x).toFixed(1) + '" y2="' + sy(b.y).toFixed(1) + '" stroke="' + stroke + '" stroke-width="2.4" stroke-linecap="round" marker-end="url(#' + markerId + ')"' + dashAttr + '/>';
    });
    // Layer 3.7: marks (ký hiệu hình học — góc vuông, cung góc, tick bằng nhau, độ dài)
    svg += renderMarksSVG(scene, layout, MAIN);
