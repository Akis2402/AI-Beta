...
(function (root) {
  ...
  function computeLayout(scene, W, H, pad) {
    ...
    var mx = (xmax - xmin) * 0.18 + 0.5, my = (ymax - ymin) * 0.18 + 0.5;
  ...
  function estimateLabelBox(cx, cy, text, anchor) {
    var w = Math.max(10, String(text).length * 8.4) + 4;
    var h = 16;
  ...
  function placeLabels(scene, layout) {
    ...
    order.forEach(function (id) {
      ...
      [16, 22, 30].forEach(function (radius) {
      ...
      candidates.forEach(function (cand) {
        ...
        pointsPx.forEach(function (pp) {
          ...
          if (d < 11) score += 250; // đè lên điểm khác
        });
  ...
  function renderSceneSVG(scene, opts) {
    ...
    (scene.visiblePoints || Object.keys(scene.points)).forEach(function (id) {
      ...
      var lp = labelPos[id];
      if (!label || !lp) return;
      svg += '<text x="' + lp.x.toFixed(1) + '" y="' + lp.y.toFixed(1) + '" font-size="16" font-family="Inter,sans-serif" fill="var(--text)" font-weight="700" text-anchor="' + lp.anchor + '" dominant-baseline="middle" paint-order="stroke" stroke="var(--paper-3)" stroke-width="4" stroke-linejoin="round">' + esc(label) + '</text>';
    });
