...
function loadAll() {
  ...
  state.docs = [];
  const docsReadyPromise = (window.docStore ? window.docStore.migrateFromLegacyIfNeeded() : Promise.resolve([]))
    .then((docs) => {
      state.docs = docs || [];
      sourceCounter = state.docs.reduce((max, d) => Math.max(max, d.id), 0);
      renderSources();
    })
    .catch(() => { state.docs = []; renderSources(); });
...
function paintStudyActions(wrapper, msgObj, answerText, aiRow) {
  ...
  wrapper.innerHTML = `
    ...
    <div class="study-actions">
      ...
      <button class="study-btn" data-act="selfcheck">${ICONS.brain}<span>Tự kiểm tra</span></button>
      <button class="study-btn" data-act="similar">${ICONS.refresh}<span>Bài tương tự</span></button>
  ...
  wrapper.querySelector('[data-act="similar"]').onclick = (e) => handleSimilarProblem(e.currentTarget, msgObj, answerText);
 ...
 * Không có endpoint riêng ở backend — dùng lại đúng pipeline /api/chat hiện có (stage 'detail')
 * với 1 prompt có cấu trúc: đề bài gốc + lời giải đúng (làm căn cứ chấm, không lộ nguyên văn) + bài
 * làm của học sinh, yêu cầu AI chấm/tìm lỗi/gợi ý thay vì đưa lại đáp án ngay. Panel mở/đóng ngay
 * tại chỗ (giữ trạng thái trên wrapper._scOpen) để không phải render lại toàn bộ câu trả lời.
 */
...
async function runSelfCheck(panel, wrapper, msgObj, answerText) {
  ...
  const prompt = `Đề bài:\n${problemText}\n\nLời giải đúng (chỉ để bạn tham khảo khi chấm, đừng chép lại nguyên văn ngay từ đầu):\n${(answerText || '').slice(0, 2000)}\n\nBài làm của học sinh:\n${attempt}\n\nHãy đóng vai gia sư chấm bài: nói rõ bài làm đúng hay sai, chỉ ra CHÍNH XÁC bước nào sai và vì sao, gợi ý cách sửa. Chỉ trình bày lại lời giải đầy đủ nếu học sinh sai từ đầu hoặc bỏ trống. Trình bày ngắn gọn, khích lệ, không lặp lại nguyên văn đề bài.`;
  try {
    const data = await apiPost('/api/chat', {
      query: prompt, stage: 'detail', deepThinking: false, crossCheck: false,
      contexts: [], rules: state.rules, settings: state.settings, history: []
    });
    resultEl.innerHTML = '';
    renderAnswerBlock(resultEl, data.text || '');
...
/* ================= Bài tương tự =================
 * Không sinh dữ liệu ở client — chỉ tạo 1 prompt có cấu trúc rồi gửi qua ĐÚNG pipeline giải bài hiện
 * có (sendMessage, vẫn Hướng giải -> Lời giải chi tiết như bình thường), để bài tương tự nhận được
 * đầy đủ Approach/Detail/Learning actions giống 1 bài giải thông thường.
 */
function handleSimilarProblem(btn, msgObj, answerText) {
  const ref = (answerText || (msgObj && msgObj.query) || '').slice(0, 1500);
  const input = el('qInput');
  input.value = `Đọc bài mình vừa giải sau và tạo cho mình 1 bài tương tự — cùng dạng, cùng kiến thức, độ khó tương đương, chỉ đổi số liệu/dữ kiện:\n\n${ref}`;
  ...
  if (used.length) {
    html += '<div class="cite-title">Tài liệu đã tải lên đã dùng</div>' +
      used.map(({ c, num }) => {
        ...
        return `<div class="cite"><b>[${num}] ${c.doc} · đoạn ${c.id}</b><br>${body}</div>`;
      }).join('');
  }
  if (webNote) {
