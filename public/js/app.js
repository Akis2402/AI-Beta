...
function appendFileMessage(kind, fileName, blob) {
  ...
  content.innerHTML = `
    <p style="margin:0 0 10px;">✅ Đã tạo xong file, sẵn sàng tải xuống bên dưới:</p>
...
el('outlineConfirmBtn').onclick = async () => {
  if (!activeOutlineCtx) return;
  ...
  try {
    ...
    closeOutlineModal();
    appendFileMessage('docx', fileName, blob);
  } catch (e) {
    console.error(e);
    alert((e && e.message) || 'Không tạo được đề cương, vui lòng thử lại.');
  } finally {
    btn.disabled = false;
...
function renderStoredAiMessage(msg) {
  const aiRow = addAiMsg(msg.outlineSpec ? 'Đề cương' : msg.mindmapSpec ? 'Mindmap' : 'Trợ Giải');
...
function renderOutlineAnswer(contentEl, spec, msgObj, aiRow) {
  ...
  dlBtn.onclick = async () => {
    ...
    try {
      ...
      console.error(e);
      alert((e && e.message) || 'Không tạo được file .docx, vui lòng thử lại.');
    } finally {
...
async function handleExportPPT(btn, answerText) {
  ...
  try {
    ...
    appendFileMessage('pptx', fileName, blob);
  } catch (e) {
    console.error(e);
...
/* ================= Đề xuất ôn tập (khung bên phải) =================
   ...
   tập uy tín rồi hiển thị ở khung nổi bên phải màn hình. REWORK: route này KHÔNG còn gọi AI/tìm
   kiếm web thật song song với lượt AI đang giải bài ở /api/chat nữa (xem comment đầu file
   server/routes/recommend.js) — chỉ tạo trực tiếp link Google giới hạn theo từng trang uy tín, trả
   về gần như tức thì. Nếu câu hỏi CHỈ xin đề (không kèm bài toán cụ thể), sendMessage() ở trên sẽ bỏ
   qua bước gọi AI giải bài, chỉ cần khung này hiện ra là đủ. */
...
function openRecommendPanel() { el('recommendPanel').classList.add('open'); el('recommendReopenBtn').classList.remove('show'); }
function closeRecommendPanel() { el('recommendPanel').classList.remove('open'); el('recommendReopenBtn').classList.add('show'); }
el('recommendCloseBtn').onclick = closeRecommendPanel;
el('recommendReopenBtn').onclick = openRecommendPanel;

// Khung "Flashcard ôn tập" — cùng cơ chế mở/đóng/nút-mở-lại với #recommendPanel ở trên (xem
// handleFlashcards() phía trên).
function openFlashcardPanel() { el('flashcardPanel').classList.add('open'); el('flashcardReopenBtn').classList.remove('show'); }
function closeFlashcardPanel() { el('flashcardPanel').classList.remove('open'); el('flashcardReopenBtn').classList.add('show'); }
el('flashcardCloseBtn').onclick = closeFlashcardPanel;
el('flashcardReopenBtn').onclick = openFlashcardPanel;

// REWORK: trước đây khung này gọi AI (tìm kiếm web thật) CHẠY SONG SONG với lượt AI đang giải bài
// ở /api/chat — tốn thêm 1 lượt gọi AI mỗi câu hỏi, có thể mất vài giây và luôn cần trạng thái
// "đang tải" (renderRecommendLoading, rec-skel...). Giờ server (server/routes/recommend.js) KHÔNG
// còn gọi AI nữa — chỉ tạo trực tiếp danh sách link Google tìm-giới-hạn-theo-trang-uy-tín, trả về
// gần như tức thì, nên bỏ luôn trạng thái loading/skeleton và ghi chú "chưa xác minh" (không còn ý
// nghĩa khi đây LUÔN LÀ cách hoạt động duy nhất, không phải phương án dự phòng khi AI thất bại).
...
function renderRecommendResults(topic, links) {
  ...
  const note = '<div class="rec-fallback-note">🔎 Bấm 1 trang bên dưới để mở kết quả tìm kiếm Google giới hạn trong đúng trang đó.</div>';
...
async function scheduleRecommend(query) {
  ...
  try {
    ...
    if (!res.ok) { renderRecommendError((data && data.error) || 'Không tìm được tài liệu liên quan.'); return; }
