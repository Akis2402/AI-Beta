  ...
  if (used.length) {
    html += '<div class="cite-title">Tài liệu đã tải lên đã dùng</div>' +
      used.map(({ c, num }) => {
        ...
        return `<div class="cite"><b>[${num}] ${escapeHtml(c.doc)} · đoạn ${escapeHtml(String(c.id))}</b><br>${body}</div>`;
      }).join('');
  }
  if (webNote) {
 ...
 * — dùng chung cho cả nút đính kèm và cơ chế dán ảnh (Ctrl+V/Cmd+V) bên dưới, tránh lặp code.
 */
// FIX ROOT CAUSE #1 (F5 làm mất ảnh đề bài): trước đây ảnh chỉ tồn tại trong RAM
// (state.pendingImage) rồi bị conversation persist bỏ qua hoàn toàn (chỉ lưu hadImage:true).
// Nay MỌI ảnh, ngay khi chọn/dán/kéo-thả, đều được lưu bền vào IndexedDB (chatImageStore) TRƯỚC
// khi state.pendingImage được set — state.pendingImage.imageId là tham chiếu duy nhất sẽ được lưu
// xuống conversation/localStorage (KHÔNG lưu base64 lớn vào localStorage — mục 2 của yêu cầu).
async function loadImageFile(file) {
  if (!file) return;
  if (!file.type || !file.type.startsWith('image/')) { alert('Chỉ hỗ trợ dán/đính kèm file ảnh.'); return; }
  if (file.size > 5 * 1024 * 1024) { alert('Ảnh vượt quá 5MB, vui lòng chọn ảnh nhỏ hơn.'); return; }

  // Nếu đang có 1 ảnh pending khác (chưa gửi) bị thay thế bởi ảnh mới này — dọn ảnh tạm cũ khỏi
  // IndexedDB luôn, tránh rác image "orphan" không bao giờ được reference (mục 9).
  ...
  const dataUrl = await new Promise((resolve, reject) => {
    ...
    reader.readAsDataURL(file);
  }).catch((e) => { console.error(e); alert('Không đọc được file ảnh, vui lòng thử lại.'); return null; });
  if (!dataUrl) return;
  ...
  if (!window.chatImageStore) {
    alert('Không thể lưu ảnh (trình duyệt không hỗ trợ lưu trữ cục bộ). Vui lòng thử trình duyệt khác.');
    return;
  }

  let imageId;
  try {
    imageId = await window.chatImageStore.save(file, { mediaType });
  } catch (e) {
    // Mục 4: KHÔNG được âm thầm mất ảnh — báo lỗi rõ ràng, cho phép user thử lại, và KHÔNG set
    // state.pendingImage (để sendMessage() không bao giờ tạo ra message hadImage=true mà thiếu imageId).
    console.error('Lưu ảnh vào IndexedDB thất bại:', e);
    alert('Không thể lưu ảnh vào bộ nhớ cục bộ (có thể do bộ nhớ trình duyệt đầy hoặc đang ở chế độ ẩn danh). Vui lòng thử lại hoặc chọn ảnh khác.');
    return;
  }

  if (oldPending && oldPending.imageId && oldPending.imageId !== imageId) {
    window.chatImageStore.delete(oldPending.imageId).catch(() => {});
  }

  state.pendingImage = { mediaType, base64: m[2], url: dataUrl, imageId };
  renderImagePreview();
}
el('attachBtn').onclick = () => el('imageInput').click();
el('imageInput').onchange = (e) => {
...
function renderImagePreview() {
  ...
  wrap.querySelector('.rm').onclick = () => {
    // Ảnh pending đã được persist vào IndexedDB ngay từ loadImageFile() — nếu user huỷ trước khi
    // gửi, dọn luôn record đó để tránh rác "orphan image" không được message nào reference (mục 9).
    if (state.pendingImage && state.pendingImage.imageId && window.chatImageStore) {
      window.chatImageStore.delete(state.pendingImage.imageId).catch(() => {});
    }
    state.pendingImage = null; renderImagePreview();
  };
}
