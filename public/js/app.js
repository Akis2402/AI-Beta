...
function drawShape(container, spec) {
  ...
  // ============ RÀNG BUỘC HÌNH HỌC CHÍNH XÁC (spec.constraints) ============
  // Đây là cơ chế khắc phục TẬN GỐC lớp lỗi "hình vẽ không đúng đề" (vd tam giác nội tiếp đường
  // tròn nhưng đỉnh vẽ ra lại không thực sự nằm trên đường tròn, chân đường cao không thẳng hàng,
  // trung điểm lệch, giao điểm 2 đường sai chỗ...). Gốc rễ: AI chỉ ƯỚC LƯỢNG toạ độ bằng "cảm giác"
  // nên luôn có sai số — trước đây hình vẽ HOÀN TOÀN tin vào số liệu đó nên sai số này lộ ra thành
  // hình sai. Giờ AI có thể khai báo THÊM quan hệ hình học thật (nằm trên đường tròn, trung điểm,
  // chân đường vuông góc, đối xứng qua tâm, giao điểm 2 đường...) qua spec.constraints; CODE sẽ tự
  // tính lại toạ độ CHÍNH XÁC bằng công thức, ĐÈ LÊN toạ độ ước lượng ban đầu của AI cho đúng điểm
  // đó — nghĩa là dù AI tính tay có lệch bao nhiêu, một khi đã khai báo đúng quan hệ thì hình vẽ ra
  // vẫn ĐÚNG TUYỆT ĐỐI. Không có "constraints" (bài cũ/spec cũ) thì bỏ qua bước này, hành vi giữ
  // nguyên như trước — không phá vỡ khả năng tương thích ngược.
  // ============ GHÉP ĐIỂM <-> NHÃN AN TOÀN ============
  // Sửa lỗi CHÍ MẠNG: trước đây ghép "points"/"labels" của MỖI element THẲNG theo chỉ số 0,1,2...
  // (labels[idx] <-> points[idx]). Khi AI trả về 2 mảng LỆCH ĐỘ DÀI — ví dụ
  // {"points":[[toạ độ A],[toạ độ D thật]],"labels":["D"]} (points dài hơn labels 1 phần tử) — cách
  // ghép cũ gán nhãn "D" cho points[0] tức TOẠ ĐỘ CỦA A, khiến điểm D/E/F/H bị "dịch" đè lên đúng vị
  // trí một đỉnh tam giác khác đã có tên. Đây không chỉ là lỗi hiển thị mà làm SAI toạ độ dùng để tính
  // các constraint phụ thuộc (foot, intersect...) phía sau, kéo theo cả chuỗi điểm bị sai theo.
  // Khắc phục: khi 2 mảng lệch độ dài, ghép "labels" với các phần tử CUỐI của "points" (bỏ qua các
  // điểm dư không có nhãn ở ĐẦU mảng) — quan sát thực tế mọi trường hợp lỗi đều theo đúng mẫu này
  // (điểm dư ở đầu luôn là toạ độ TRÙNG LẶP của một điểm đã có tên khác, không phải điểm mới).
  function pairedPointsLabels(elmt) {
    if (!elmt || !Array.isArray(elmt.points)) return [];
    const points = elmt.points;
    const labels = Array.isArray(elmt.labels) ? elmt.labels : [];
    const offset = Math.max(0, points.length - labels.length);
    return points.map((point, idx) => {
      const labelIdx = idx - offset;
      const label = labelIdx >= 0 ? (labels[labelIdx] || '') : '';
      return { point, label, idx };
    });
  }
  function findPointOccurrences(label) {
    const occ = [];
    elements.forEach((elmt) => {
      pairedPointsLabels(elmt).forEach(({ point, label: lb, idx }) => {
        if (lb === label && point) occ.push({ elmt, idx });
      });
    });
    return occ;
  }
  function getPointCoord(label) {
    const occ = findPointOccurrences(label);
    if (!occ.length) return null;
    const p = occ[0].elmt.points[occ[0].idx];
    return [Number(p[0]), Number(p[1])];
  }
  function setPointCoord(label, xy) {
    findPointOccurrences(label).forEach(({ elmt, idx }) => { elmt.points[idx] = [xy[0], xy[1]]; });
  }
  if (spec && Array.isArray(spec.constraints)) {
    spec.constraints.forEach((c) => {
      if (!c || !c.type || !c.point) return;
      try {
        if (c.type === 'onCircle') {
          // Ép điểm nằm ĐÚNG trên đường tròn tham chiếu — giữ nguyên GÓC (hướng) từ toạ độ ước lượng
          // ban đầu của AI, chỉ chỉnh lại bán kính cho khớp — dùng cho MỌI đỉnh tam giác/đa giác nội
          // tiếp đường tròn.
          const circleElmt = elements[c.circle];
          const cur = getPointCoord(c.point);
          if (circleElmt && circleElmt.center && circleElmt.radius != null && cur) {
            const [ccx, ccy] = circleElmt.center.map(Number), r = Number(circleElmt.radius);
            let ang = Math.atan2(cur[1] - ccy, cur[0] - ccx);
            if (!isFinite(ang)) ang = 0;
            setPointCoord(c.point, [ccx + r * Math.cos(ang), ccy + r * Math.sin(ang)]);
          }
        } else if (c.type === 'midpoint' && Array.isArray(c.of) && c.of.length === 2) {
          const a = getPointCoord(c.of[0]), b = getPointCoord(c.of[1]);
          if (a && b) setPointCoord(c.point, [(a[0] + b[0]) / 2, (a[1] + b[1]) / 2]);
        } else if (c.type === 'reflect') {
          // Đối xứng qua tâm 1 đường tròn ("through" là số = chỉ số element đường tròn) HOẶC qua 1
          // điểm khác ("through" là chuỗi = nhãn điểm) — dùng cho đường kính AK (K đối xứng A qua O).
          const p = getPointCoord(c.of);
          let center = null;
          if (typeof c.through === 'number') {
            const ce = elements[c.through];
            if (ce && ce.center) center = ce.center.map(Number);
          } else if (typeof c.through === 'string') {
            center = getPointCoord(c.through);
          }
          if (p && center) setPointCoord(c.point, [2 * center[0] - p[0], 2 * center[1] - p[1]]);
        } else if (c.type === 'foot' && Array.isArray(c.segment) && c.segment.length === 2) {
          // Chân đường vuông góc hạ từ "from" xuống đoạn "segment" — dùng cho chân đường cao D,E,F.
          // Mặc định ép nằm TRONG đoạn (0<=t<=1); truyền "clamp":false nếu chân nằm ngoài đoạn (tam
          // giác tù) là đúng ý đề.
          const from = getPointCoord(c.from);
          const m = getPointCoord(c.segment[0]), n = getPointCoord(c.segment[1]);
          if (from && m && n) {
            const mx = n[0] - m[0], my = n[1] - m[1];
            const len2 = mx * mx + my * my;
            let t = len2 > 1e-9 ? ((from[0] - m[0]) * mx + (from[1] - m[1]) * my) / len2 : 0;
            if (c.clamp !== false) t = Math.max(0, Math.min(1, t));
            setPointCoord(c.point, [m[0] + t * mx, m[1] + t * my]);
          }
        } else if (c.type === 'intersect' && Array.isArray(c.line1) && c.line1.length === 2 && Array.isArray(c.line2) && c.line2.length === 2) {
          // Giao điểm CHÍNH XÁC của 2 đường thẳng đi qua 2 cặp điểm đã có — dùng cho MỌI điểm giao
          // (đường cao, đường kính, đường trung tuyến, đường phân giác nối 2 điểm đã biết...).
          const a = getPointCoord(c.line1[0]), b = getPointCoord(c.line1[1]);
          const cP = getPointCoord(c.line2[0]), d = getPointCoord(c.line2[1]);
          if (a && b && cP && d) {
            const x1 = a[0], y1 = a[1], x2 = b[0], y2 = b[1];
            const x3 = cP[0], y3 = cP[1], x4 = d[0], y4 = d[1];
            const denom = (x1 - x2) * (y3 - y4) - (y1 - y2) * (x3 - x4);
            if (Math.abs(denom) > 1e-9) {
              const px = ((x1 * y2 - y1 * x2) * (x3 - x4) - (x1 - x2) * (x3 * y4 - y3 * x4)) / denom;
              const py = ((x1 * y2 - y1 * x2) * (y3 - y4) - (y1 - y2) * (x3 * y4 - y3 * x4)) / denom;
              setPointCoord(c.point, [px, py]);
            }
          }
        } else if (c.type === 'onLine' && Array.isArray(c.through) && c.through.length === 2) {
          // Ép điểm thẳng hàng với 2 điểm đã biết (hình chiếu vuông góc của toạ độ ước lượng lên
          // đường thẳng đó) — dùng khi biết P thẳng hàng với A,B nhưng chưa cần vuông góc/giao điểm.
          const cur = getPointCoord(c.point);
          const m = getPointCoord(c.through[0]), n = getPointCoord(c.through[1]);
          if (cur && m && n) {
            const mx = n[0] - m[0], my = n[1] - m[1];
            const len2 = mx * mx + my * my;
            const t = len2 > 1e-9 ? ((cur[0] - m[0]) * mx + (cur[1] - m[1]) * my) / len2 : 0;
            setPointCoord(c.point, [m[0] + t * mx, m[1] + t * my]);
          }
        }
      } catch (e) { /* Ràng buộc lỗi cú pháp -> bỏ qua, giữ nguyên toạ độ AI đã ước lượng cho điểm đó */ }
    });
  }
  ...
  // Nhãn được TÍNH VỊ TRÍ TRƯỚC (chưa vẽ text ngay) rồi mới chống-chồng-nhãn ở vòng riêng bên dưới —
  // vì khi AI trả toạ độ 2 điểm KHÁC TÊN trùng hoặc gần trùng nhau (vd chân đường cao tính lệch ra
  // đúng vị trí một đỉnh), nhãn cũ luôn lệch cố định +18px cùng hướng nên 2 chữ đè khít lên nhau,
  // đọc nhầm thành 1 từ ghép (vd "AA", "BE" thay vì "A" và "D" tách biệt). Chấm điểm vẫn vẽ ngay.
  const labelEntries = [];
  // Sửa lỗi CHÍ MẠNG thứ 2: chồng nhãn kiểu "A A", "C C". Nguyên nhân — AI đôi khi khai báo LẶP LẠI
  // y hệt 1 điểm/nhãn ở NHIỀU element khác nhau trong "elements" (vd toạ độ+nhãn A,B,C xuất hiện cả
  // trong "polygon" LẪN trong 1 "points" riêng bên dưới); trước đây mỗi lần lặp lại đều bị vẽ thêm 1
  // chấm + 1 chữ chồng lên chấm/chữ trước đó. Khắc phục: mỗi NHÃN chỉ được vẽ (chấm + chữ) ĐÚNG 1 LẦN
  // DUY NHẤT — ở lần xuất hiện ĐẦU TIÊN trong "elements" — các lần lặp lại sau đó bị bỏ qua. Đồng thời
  // bỏ qua hẳn các điểm KHÔNG có nhãn (dư ra do 2 mảng points/labels lệch độ dài ở pairedPointsLabels
  // phía trên) thay vì vẽ thành chấm "ma" không tên gây rối hình.
  const drawnLabels = new Set();
  elements.forEach((elmt) => {
    pairedPointsLabels(elmt).forEach(({ point: p, label }) => {
      if (!label || drawnLabels.has(label)) return;
      ...
      // sy() đảo trục dọc nên "-dy" mới đúng hướng lên
      labelEntries.push({ px, py, dx, dy, label: String(label), lx: px + dx * 18, ly: py - dy * 18 });
    });
  });

  // Chống chồng nhãn: nới dần khoảng cách giữa các nhãn ở gần nhau (kể cả khi 2 điểm gần/trùng toạ
  // độ) cho tới khi mỗi nhãn cách nhãn khác ít nhất MINSEP px — đảm bảo LUÔN đọc tách biệt từng tên
  // điểm, không phụ thuộc AI tính toạ độ có chính xác tuyệt đối hay không.
  const MINSEP = 20;
  for (let iter = 0; iter < 8; iter++) {
    let moved = false;
    for (let a = 0; a < labelEntries.length; a++) {
      for (let b = a + 1; b < labelEntries.length; b++) {
        const A = labelEntries[a], B = labelEntries[b];
        let vx = B.lx - A.lx, vy = B.ly - A.ly;
        let dist = Math.hypot(vx, vy);
        if (dist < MINSEP) {
          moved = true;
          if (dist < 1e-3) {
            // 2 nhãn gần như trùng hệt nhau (2 điểm khác tên trùng toạ độ) -> tách theo hướng khác
            // nhau dựa trên chỉ số, để fan ra thay vì chồng khít lên nhau.
            const ang = ((a * 47 + b * 89) % 360) * Math.PI / 180;
            vx = Math.cos(ang); vy = Math.sin(ang); dist = 1;
          } else { vx /= dist; vy /= dist; }
  ...
  labelEntries.forEach((e) => {
    const anchor = e.dx > 0.3 ? 'start' : e.dx < -0.3 ? 'end' : 'middle';
    svg += `<text x="${e.lx.toFixed(1)}" y="${e.ly.toFixed(1)}" font-size="16" font-family="Inter,sans-serif" fill="var(--text)" font-weight="700" text-anchor="${anchor}" dominant-baseline="middle" paint-order="stroke" stroke="var(--paper-3)" stroke-width="4" stroke-linejoin="round">${e.label.replace(/</g, '&lt;')}</text>`;
  });
