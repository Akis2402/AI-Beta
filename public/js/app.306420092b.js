...
function sourceReadinessSummary() {
  ...
  return {
    ...
    // PHẦN III/VII: "processing" KHÔNG còn có nghĩa "chưa dùng được" — chỉ có nghĩa "nền vẫn ch��y".
...
function collectAvailableEvidence(query) {
  ...
  (state.urlSources || []).forEach((s) => {
    (s.chunks || []).forEach((ch, i) => {
      ...
      let score = 0;
      qWords.forEach((w) => { if (norm.includes(w)) score += 1; });
      // PHẦN BV (V6): locator đầy đủ cho cả YouTube (mốc mm:ss) lẫn Web (heading section)
      // để trích nguồn cấp địa chỉ đúng câu/đoạn, không chỉ tới cả video/trang. Vẫn nhét
      // vào đầu text để model thấy ngay khi đọc; đồng thời truyền RIÊNG qua sourceUrl/timeStart/
      // timeEnd/sectionAnchor để promptBuilder.formatContextLine() (server V6) sinh dòng "(Nguồn:
      // [YouTube] <url>, mốc mm:ss)" đúng chuẩn — và client-side renderCitations cũng dùng được.
      let prefix = '';
      ...
      out.push({
        ...
        extractionStatus: 'ok',
        // V6 — metadata bổ sung cho locator nguồn URL. Server (promptBuilder.formatContextLine) và
        // client (renderCitations) cả hai đọc được 5 trường này để in đúng địa chỉ trích dẫn.
    ...
    html += '<div class="cite-title">Tài liệu đã tải lên đã dùng</div>' +
      used.map(({ c, num }) => {
        ...
        const pageLabel = c.page != null
          ...
          : '';
        // V6 — in địa chỉ trích dẫn theo loại nguồn: PDF -> trang, YouTube -> link&t=Xs, Web ->
        // link + heading. Phần meta nằm NGAY sau tiêu đề [n] để người đọc mở được ngay vị trí.
        let meta = '';
        if (c.kind === 'youtube' && c.sourceUrl) {
          const startS = Math.max(0, Math.floor(Number(c.timeStart) || 0));
          const jumpUrl = c.sourceUrl.includes('?')
            ? `${c.sourceUrl}&t=${startS}s`
            : `${c.sourceUrl}?t=${startS}s`;
          const mmss = `${Math.floor(startS / 60)}:${String(startS % 60).padStart(2, '0')}`;
          meta = ` · <a href="${escapeHtml(jumpUrl)}" target="_blank" rel="noopener">mốc ${mmss}</a>`;
        } else if (c.kind === 'web' && c.sourceUrl) {
          const anchor = c.sectionAnchor ? ` · đoạn “${escapeHtml(c.sectionAnchor)}”` : '';
          meta = ` · <a href="${escapeHtml(c.sourceUrl)}" target="_blank" rel="noopener">mở trang</a>${anchor}`;
        } else if (pageLabel) {
          meta = pageLabel; // đã có " · trang X" snẵn
        }
...
async function fetchDetail(btn, aiRow, contentEl, msgObj, images) {
  ...
  try {
    const data = await streamViaProviderRouter('/api/chat', {
      ...
      // MỤC 33/43: gửi THAM CHIẾU hình của lượt Hướng giải (chỉ visualId — không gửi lại URL/base64).
      // Server dùng nó để DÙNG LẠI đúng hình đó; stage 'detail' không bao giờ sinh hình mới nữa, kể
      // cả khi trường này vắng mặt (khoá nằm ở server — xem visualPipeline.stageMayGenerate).
      visualId: (msgObj.approachVisuals && msgObj.approachVisuals[0] && msgObj.approachVisuals[0].visualId) || undefined
    }, {
      onDelta: (piece) => { if (taskHandle) ctm.appendDelta(taskHandle.task.requestId, piece); preview.append(piece); scrollThreadToBottom(); },
      onStatus: (msg, st) => { if (taskHandle) ctm.setStatus(taskHandle.task.requestId, msg, st); preview.setStatus(msg, st); },
