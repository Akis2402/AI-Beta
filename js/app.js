...
function renderSources() {
  ...
  state.docs.forEach((doc) => {
    ...
    if (doc.sourceType === 'image-pdf' && doc.pageCoverage) {
      ...
      coverageLabel = pc.coveragePercent >= 100
        ? `${pc.totalPages}/${pc.totalPages} trang · OCR/Vision hoàn tất`
        : `${pc.renderedPages}/${pc.totalPages} trang · ${pc.coveragePercent}% · chưa hoàn tất`;
    } else {
...
function collectSourceImages(query = '') {
  ...
  imagePdfDocs.forEach((doc) => {
    ...
    const picked = doc.pageImages.filter((img) => pageHints.has(img.page));
    if (picked.length < perDocBudget) {
      const remain = perDocBudget - picked.length;
      const already = new Set(picked.map((img) => img.page));
      const rest = doc.pageImages.filter((img) => !already.has(img.page));
  ...
  return out.slice(0, MAX_TOTAL_SOURCE_IMAGES);
}
