...
//   2. Xong — không cần khởi động lại code, không cần sửa gì thêm, KHÔNG cần điền model. Lần request
//      kế tiếp, server/utils/modelDiscovery.js tự gọi endpoint /models của hãng đó (suy ra từ
//      `baseURL`, hoặc `discoveryURL` nếu khai riêng) để lấy danh sách model THẬT, tự chọn model
//      mạnh + model nhanh phù hợp, rồi server/utils/aiProviders.js đưa provider này vào vòng xoay
//      ngẫu nhiên của cả chế độ "Nhanh" lẫn "Suy nghĩ sâu".
