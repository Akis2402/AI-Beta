...
router.post('/ppt-outline', async (req, res, next) => {
  try {
    ...
    // maxTokens trước đây (1400, rồi 2800) vẫn có thể chưa đủ cho slide nhiều nội dung/nhiều
    // bullet tiếng Việt — tăng thêm lên mức rộng rãi hơn theo yêu cầu, giảm tối đa nguy cơ cắt
    // ngang giữa chừng trước khi kịp đóng xong JSON.
    // Tăng thêm lần nữa theo yêu cầu người dùng (trước đây 4200) — vẫn cùng lý do: bài có nhiều
    // slide (tới ~12) x nhiều bullet tiếng Việt dài dễ chạm trần cũ, JSON dở dang thì lớp vá
    // closeTruncatedJSON chỉ cứu được cấu trúc chứ không phục hồi được nội dung đã mất.
