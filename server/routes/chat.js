...
router.post('/', async (req, res, next) => {
  ...
  try {
      ...
      try {
        if (input.crossCheck && input.stage === 'detail') {
          ...
          try {
            const gathered = await gatherCrossCheckCandidates(activeProviders, {
              system, variantSystem, messages, maxTokens: 2400,
          ...
          // TRƯỚC ĐÂY: web search CHỈ được bật khi HOÀN TOÀN không có context nào (input.contexts.
          // length === 0) — nghĩa là 1 bài có tài liệu nhưng tài liệu đó chỉ cung cấp MỘT PHẦN thông
          // tin cần thiết thì AI KHÔNG BAO GIỜ được phép tra cứu thêm web để bổ sung phần còn thiếu,
          // dù có thể đang thiếu ngay 1 công thức quan trọng. Đây là logic nhị phân sai với nguyên
          // tắc "ưu tiên tài liệu người dùng trước, web chỉ khi thiếu" — nó chặn cả trường hợp cần bổ
          // sung, không chỉ chặn trường hợp lạm dụng web khi đã đủ tài liệu.
          // NAY: luôn cấp công cụ web search cho lượt TỔNG HỢP (nếu provider hỗ trợ) — việc CÓ THỰC
          // SỰ DÙNG web hay không, và chỉ dùng để bổ sung phần thiếu, do prompt quyết định (xem
          // buildSourcePolicyBlock trong promptBuilder.js: ưu tiên tuyệt đối đoạn trích tài liệu
          // trước, chỉ được dùng web cho phần thực sự còn thiếu, và phải tự đánh dấu rõ khi có dùng).
          const hasWebSearch = true;
          ...
          const { provider: reconciler } = await streamWithFailover(
            activeProviders,
            { system: reconcileSystem, messages, maxTokens: 3300, webSearch: hasWebSearch, timeoutMs: RECONCILE_TIMEOUT_MS },
        ...
        const { provider } = await streamWithFailover(
          activeProviders,
          {
            ...
            maxTokens: input.stage === 'approach'
              ? (input.deepThinking ? 3300 : 2100)
              : (input.deepThinking ? 6000 : 3000),
            fast: true
          },
          (piece) => { full += piece; sseWrite(res, 'delta', { text: piece }); }
        );
    ...
    if (input.crossCheck && input.stage === 'detail') {
      ...
      // Xem giải thích đầy đủ ở nhánh streaming phía trên — cùng lý do, cùng logic.
      const hasWebSearch = true;
      const reconcileSystem = buildReconcileSystemPrompt({
        // candidates ở đây đã được strip <thinking>/<think> ngay từ gatherCrossCheckCandidates()
        // (xem server/utils/aiProviders.js) — không cần strip lại ở đây.
    ...
    const { text, provider } = await callFastest(activeProviders, {
      ...
      // sâu (deepBlock vẫn được nối vào cả stage approach, xem promptBuilder.js).
      maxTokens: input.stage === 'approach'
        ? (input.deepThinking ? 3300 : 2100)
        : (input.deepThinking ? 6000 : 3000),
      fast: true
