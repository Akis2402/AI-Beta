 ...
 */
async function callOpenAI({ system, messages, maxTokens = 1000, temperature, webSearch, fast, timeoutMs = DEFAULT_TIMEOUT_MS }) {
  if (!API_KEY) {
 ...
 * @returns {Promise<string>}
 */
async function callOpenAIStream({ system, messages, maxTokens = 1000, temperature, webSearch, fast, timeoutMs = DEFAULT_TIMEOUT_MS, onDelta }) {
  if (!API_KEY) {
    const err = new Error('Máy chủ chưa cấu hình OPENAI_API_KEY.');
    err.status = 500;
    throw err;
