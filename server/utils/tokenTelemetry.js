...
const { getCacheDiscount } = require('./pricing/requestCostCalculator');
...
class RequestTokenTelemetry {
  constructor(requestId) {
    ...
    this.totals = {
      ...
      imageInputTokens: 0,       // Token ảnh đầu vào
      imageOutputTokens: 0,      // Token ảnh đầu ra
      toolUseTokens: 0,          // Token thực thi tool call
  ...
  recordAttempt(a = {}) {
    ...
    const record = {
      ...
      imageInputTokens: hasRealUsage && usage.imageInputTokens != null ? num(usage.imageInputTokens) : null,
      imageOutputTokens: hasRealUsage && usage.imageOutputTokens != null ? num(usage.imageOutputTokens) : null,
      toolUseTokens: hasRealUsage && usage.toolUseTokens != null ? num(usage.toolUseTokens) : null,
    ...
    if (hasRealUsage) {
      ...
      t.imageInputTokens += record.imageInputTokens || 0;
      t.imageOutputTokens += record.imageOutputTokens || 0;
      t.toolUseTokens += record.toolUseTokens || 0;
    ...
    t.actualTokens = t.inputTokens + t.outputTokens + t.reasoningTokens + t.imageInputTokens + t.imageOutputTokens + t.toolUseTokens;
    // Token đọc từ cache được tính giá theo tỷ lệ chiết khấu của provider/model
    const discount = typeof getCacheDiscount === 'function' ? getCacheDiscount(record.provider, record.model) : 0.75;
    t.estimatedSavings = Math.round(t.cachedTokens * discount);
    return record;
  }
