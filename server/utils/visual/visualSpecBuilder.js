...
function buildImagePrompt(spec, opts = {}) {
  ...
  // Mức MEDIUM / BUDGET CẮT: nén theo thứ tự ưu tiên ngữ nghĩa (Section 38), KHÔNG cắt mù ở giữa câu/từ
  ...
  if (budgetForBody > 0 && body.length > budgetForBody) {
    // Semantic priority: Giữ nguyên exact facts, objects, labels, relationships, math constraints
    // Rút gọn các dòng phong cách, mô tả phụ trước
    const criticalPrefixes = [
      'Exact facts:',
      'Required objects:',
      'Required labels:',
      'Spatial relationships:',
      'Math/measurement constraints:'
    ];

    const criticalLines = lines.filter(l => criticalPrefixes.some(p => l.startsWith(p)));
    const secondaryLines = lines.filter(l => !criticalPrefixes.some(p => l.startsWith(p)));

    // Rút gọn style trước
    const shortenedSecondary = secondaryLines.map(l => {
      if (l.startsWith('Style:')) return 'Style: Clean educational illustration.';
      return l;
    });

    let candidateLines = [...criticalLines, ...shortenedSecondary];
    body = candidateLines.join('\n');

    // Nếu vẫn vượt, bỏ bớt secondary lines từ cuối
    while (body.length > budgetForBody && shortenedSecondary.length > 0) {
      shortenedSecondary.pop();
      candidateLines = [...criticalLines, ...shortenedSecondary];
      body = candidateLines.join('\n');
    }

    // Nếu chỉ còn critical lines mà vẫn vượt ngân sách quá chặt: cắt tại ranh giới từ an toàn
    if (body.length > budgetForBody) {
      const truncated = body.slice(0, budgetForBody);
      const lastSpace = truncated.lastIndexOf(' ');
      body = (lastSpace > budgetForBody * 0.7 ? truncated.slice(0, lastSpace) : truncated).trimEnd();
    }
  }
  return `${body}\n${tail}`;
}
