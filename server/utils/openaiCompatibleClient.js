...
function createOpenAICompatibleClient(config) {
   ...
   */
  async function call({ system, messages, maxTokens = 1000, temperature, fast, deepThinking, timeoutMs = 30000, apiKeyOverride, modelOverride, fastModelOverride, signal }) {
   ...
   */
  async function callStream({ system, messages, maxTokens = 1000, temperature, fast, deepThinking, timeoutMs = 30000, onDelta, apiKeyOverride, modelOverride, fastModelOverride, signal }) {
    const key = apiKeyOverride || apiKey();
    if (!key) {
      const err = new Error(`Máy chủ chưa cấu hình ${apiKeyEnv} cho nhà cung cấp ${label}.`);
      err.status = 500;
