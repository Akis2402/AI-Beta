...
function createOpenAICompatibleClient(config) {
   ...
   */
  async function call({ system, messages, maxTokens = 1000, temperature, fast, timeoutMs = 30000, apiKeyOverride, modelOverride, fastModelOverride }) {
   ...
   */
  async function callStream({ system, messages, maxTokens = 1000, temperature, fast, timeoutMs = 30000, onDelta, apiKeyOverride, modelOverride, fastModelOverride }) {
    const key = apiKeyOverride || apiKey();
    if (!key) {
      const err = new Error(`Máy chủ chưa cấu hình ${apiKeyEnv} cho nhà cung cấp ${label}.`);
      err.status = 500;
