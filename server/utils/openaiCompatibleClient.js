...
function createOpenAICompatibleClient(config) {
  ...
  /**
   * @param {{system:string, messages:Array, maxTokens?:number, temperature?:number, fast?:boolean, timeoutMs?:number}} opts
   */
  async function call({ system, messages, maxTokens = 1000, temperature, fast, timeoutMs = 30000 }) {
    const key = apiKey();
    if (!key) {
   ...
   */
  async function callStream({ system, messages, maxTokens = 1000, temperature, fast, timeoutMs = 30000, onDelta }) {
    const key = apiKey();
    if (!key) {
      const err = new Error(`Máy chủ chưa cấu hình ${apiKeyEnv} cho nhà cung cấp ${label}.`);
      err.status = 500;
