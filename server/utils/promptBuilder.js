...
function buildChatSystemPrompt({ deepThinking, image, rules, contexts, settings, stage, approachText, problemText = '' }) {
...
${FORMAT_INSTRUCTIONS}
${buildSourcePolicyBlock({ hasContexts: contexts.length > 0, hasWebSearch: false })}${drawingNeeded ? buildDrawInstructions({ stageLabel: 'hướng giải' }) : NO_DRAWING_NOTE}${deepBlock}${imageBlock}${rulesBlock}${contextBlock}`;
  }
...
${buildSourcePolicyBlock({ hasContexts: contexts.length > 0, hasWebSearch: false })}${drawingNeeded ? '\n' + buildDrawInstructions({ stageLabel: 'lời giải chi tiết' }) : NO_DRAWING_NOTE}${deepBlock}${imageBlock}${rulesBlock}${approachBlock}${contextBlock}`;
}
...
function buildReconcileSystemPrompt({ candidates, contexts, settings, hasWebSearch, deepThinking, agreement }) {
...
const PROMPT_VERSION = 'chat-prompt-v3';
