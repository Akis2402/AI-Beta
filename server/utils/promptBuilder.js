...
\`\`\`
     ...
     labels: mảng tên đỉnh BẮT BUỘC theo đúng thứ tự đỉnh của khối, khớp với ký hiệu dùng trong lời giải (S, A, B, C, D...) — CHỈ áp dụng cho khối PRIMITIVE nguyên vẹn ở trên.

     ⚠️ KHỐI BỊ BIẾN ĐỔI (cắt/khoét/xuyên lỗ/ghép/bỏ góc/cắt bởi mặt phẳng...) — BẮT BUỘC đọc mục 6E bên dưới trước khi vẽ loại này. TUYỆT ĐỐI KHÔNG được biểu diễn một khối ĐÃ BỊ BIẾN ĐỔI bằng đúng 1 primitive nguyên bản ở trên (vd "hình lập phương bị cắt một góc" KHÔNG được vẽ bằng {"type":"cube"} trơn) — dùng đúng 1 trong 3 "type" composable sau:
     - "transform" (dịch chuyển/xoay 1 khối con): {"type":"transform","position":[x,y,z],"rotation":[rx,ry,rz],"child":{...bất kỳ node solid3d nào...}}
...
// ---------- FIX PHẦN 5 (prompt compression): DRAW_SCHEMA TUYỆT ĐỐI KHÔNG được gửi vô điều kiện ----------
// TRƯỚC ĐÂY buildDrawInstructions() (kèm theo TOÀN BỘ DRAW_SCHEMA — mẫu JSON cho cả shape/solid3d/
// plot, rất dài) được nối vào MỌI system prompt bất kể đề bài có liên quan gì tới hình vẽ hay không
// (vd 1 bài đại số/số học thuần tuý vẫn phải nhận toàn bộ hướng dẫn 3D/composite/boolean...) — đây
// là dead weight token lớn nhất trong prompt. Chỉ gửi khối này khi có tín hiệu THỰC SỰ cần vẽ hình
// (hình học phẳng/không gian, đồ thị, toạ độ...), khi đề có ẢNH (không thể biết trước nội dung ảnh
// bằng text-match nên PHẢI an toàn, gửi đầy đủ), hoặc khi approach trước đó đã có sẵn 1 khối vẽ cần
// giữ nhất quán (detail stage) — ưu tiên CORRECTNESS hơn tiết kiệm token (mục 21).
const GEOMETRY_OR_DRAWING_RE = /tam giác|tứ giác|ngũ giác|lục giác|đa giác|hình bình hành|hình thang|hình chữ nhật|hình vuông|hình thoi|đường tròn|hình tròn|hình chóp|lăng trụ|hình hộp|khối chóp|khối lăng trụ|hình nón|khối nón|hình trụ|khối trụ|hình cầu|mặt cầu|khối cầu|khối tròn xoay|đồ thị|parabol|elip|hypebol|hyperbol|toạ độ|tọa độ|vuông góc|trung điểm|trung tuyến|đường cao|đường phân giác|đường trung trực|nội tiếp|ngoại tiếp|diện tích|thể tích|chu vi|bán kính|đường kính|mặt phẳng|không gian oxyz|oxyz|vector|véc\s*tơ|đỉnh|cạnh bên|góc giữa|khoảng cách từ|hệ trục/i;

/** @returns {boolean} true nếu văn bản có tín hiệu thực sự cần vẽ hình/đồ thị (mục 5). */
...
BẢNG SO SÁNH: nếu câu trả lời có từ 2 đối tượng/phương án/trường hợp/đại lượng/kết quả trở lên cần ĐỐI CHIẾU với nhau (nhiều cách giải, nhiều trường hợp của bài toán, ưu/nhược điểm, trước/sau, các đại lượng kèm đơn vị...), hãy trình bày bằng bảng Markdown chuẩn (| cột | cột |, dòng phân cách |---|---|) thay vì liệt kê dài dòng — giúp quét mắt nhanh, khoa học. KHÔNG dùng bảng nếu chỉ có 1 kết quả đơn giản, các bước cần trình bày tuần tự, hoặc không có ít nhất 2 nhóm để đối chiếu — khi đó dùng đoạn văn/gạch đầu dòng bình thường. Số trong bảng giữ nguyên độ chính xác, không tự làm tròn; đơn vị nhất quán trong cùng một cột. Sau bảng, nêu ngắn gọn kết luận/lựa chọn nên dùng, KHÔNG lặp lại nội dung bảng.`;
...
// Nhãn hiển thị PHẢI khớp window.GRADE_LABELS / window.SCHOOL_LEVELS ở public/js/formulas.js —
// đây là bản sao phía server (2 runtime tách biệt — browser vs Node — nên không import chung
// module được, nhưng phải đồng bộ thủ công nếu sau này đổi nhãn ở 1 trong 2 nơi).
const SCHOOL_LABELS = { 'tieu-hoc': 'Tiểu học', 'thcs': 'THCS', 'thpt': 'THPT', 'dai-hoc': 'Đại học' };
const GRADE_LABELS = {
  '1': 'Lớp 1', '2': 'Lớp 2', '3': 'Lớp 3', '4': 'Lớp 4', '5': 'Lớp 5',
  '6': 'Lớp 6', '7': 'Lớp 7', '8': 'Lớp 8', '9': 'Lớp 9',
  '10': 'Lớp 10', '11': 'Lớp 11', '12': 'Lớp 12', 'dai-hoc': 'Đại học'
};

// Chỉ thị cấp học/lớp — chèn vào MỌI system prompt sinh lời giải (approach/detail/reconcile) để
// school/grade thực sự "đi tới" AI thay vì chỉ nằm ở UI (trước bản sửa này, validateChatBody() bỏ
// hẳn 2 trường này nên khối này chưa từng tồn tại). Yêu cầu AI: (1) chọn công thức/định lý đúng
// đúng cấp đang học; (2) không dùng kiến thức vượt cấp trừ khi người dùng chủ động hỏi mở rộng;
// (3) dùng notation/mức độ trình bày phù hợp lứa tuổi/lớp.
function buildSchoolGradeDirective(school, grade) {
  if (!school || !grade) return '';
  const schoolLabel = SCHOOL_LABELS[school] || school;
  const gradeLabel = GRADE_LABELS[grade] || grade;
  return `\n\nHỌC SINH ĐANG HỌC: ${schoolLabel}${grade !== 'dai-hoc' ? ', ' + gradeLabel : ''}. BẮT BUỘC: chỉ dùng công thức/định lý/phương pháp thuộc đúng chương trình ${schoolLabel}${grade !== 'dai-hoc' ? ' ' + gradeLabel : ''} để giải; nếu cách giải tự nhiên nhất đòi hỏi kiến thức VƯỢT CẤP (chưa học ở lớp này), hãy ưu tiên cách giải bằng kiến thức đúng cấp trước — chỉ nêu thêm cách dùng kiến thức vượt cấp như một ghi chú phụ, không thay thế cách giải chính. Dùng notation và mức độ trình bày phù hợp với học sinh ${schoolLabel}${grade !== 'dai-hoc' ? ' ' + gradeLabel : ''}.`;
}
...
// ---------- Mục 1: CHỈ CÒN 2 mức độ chi tiết ("ngắn gọn" / "tiêu chuẩn") ----------
// Trước đây chỉ có 1 câu ngắn "Mức độ chi tiết mong muốn: X" — không định nghĩa rõ 2 mức khác nhau
// THẾ NÀO, khiến model dễ trả lời dài như nhau bất kể mức chọn. Định nghĩa RÕ mỗi mức, và nhắc rõ
// không được lặp lại nội dung Hướng giải một cách máy móc (đây là nguyên nhân phổ biến khiến
// "tiêu chuẩn" bị dài dòng thừa thãi). Không còn logic riêng nào cho 'rất chi tiết' (đã loại bỏ).
function buildDetailModeDirective(detail) {
  if (detail === 'ngắn gọn') {
    return `Mức độ chi tiết mong muốn: NGẮN GỌN — chỉ trình bày tóm tắt tối thiểu: các bước THỰC SỰ cần thiết, phép tính quan trọng, và kết luận. Không giải thích lan man, không diễn giải lại lý do hiển nhiên, không lặp ý.`;
  }
  return `Mức độ chi tiết mong muốn: TIÊU CHUẨN — trình bày đầy đủ từng bước cần thiết kèm công thức/định lý dùng tới, các biến đổi quan trọng, và kết luận rõ ràng. KHÔNG lặp lại y nguyên nội dung đã nêu ở "Hướng giải" (nếu người dùng đã xem hướng giải trước đó) một cách máy móc — hãy TRIỂN KHAI CHI TIẾT (tính toán, biến đổi cụ thể) thay vì diễn đạt lại cùng những gạch đầu dòng định hướng.`;
}
...
${buildSourcePolicyBlock({ hasContexts: contexts.length > 0, hasWebSearch: false })}${drawingNeeded ? buildDrawInstructions({ stageLabel: 'hướng giải' }) : NO_DRAWING_NOTE}${subjectBlock}${deepBlock}${imageBlock}${rulesBlock}${contextBlock}`;
  ...
  // "kéo" model quay lại ngôn ngữ cũ — đây là 1 trong các nguồn input tiếng Việt có thể làm rò rỉ
  // ngôn ngữ vào đầu output detail mà buildLanguageDirective() đặt ở đầu prompt không đủ để chặn nếu
  // không nhắc lại ngay tại điểm chèn nội dung tiếng Việt.
  const approachLangReminder = `\n(Lưu ý: đoạn hướng giải tham khảo trên có thể được viết bằng ngôn ngữ KHÁC ngôn ngữ trả lời đã chỉ định — CHỈ dùng nó để lấy Ý và hình vẽ, KHÔNG được để ngôn ngữ của nó ảnh hưởng tới ngôn ngữ câu trả lời chi tiết; toàn bộ câu trả lời chi tiết vẫn phải 100% đúng ngôn ngữ đã chỉ định ở trên ngay từ câu đầu tiên.)`;
  const approachBlock = approachText
    ? `\n\nBạn (AI) đã gợi ý HƯỚNG GIẢI sau đây cho người dùng ở bước trước, người dùng vừa bấm "Xem cách giải chi tiết":\n---\n${approachText}\n---${approachLangReminder}\nHãy triển khai chi tiết NHẤT QUÁN với hướng giải này; nếu trong lúc giải chi tiết phát hiện hướng trên có sai sót, được phép điều chỉnh nhưng phải nêu rõ vì sao trong phần "Lời giải".` +
      (approachDrawMatch
...
// ---------- FIX PHẦN 2 (cache fingerprint): version của bộ prompt hiện tại ----------
// Tăng số này mỗi khi nội dung/CẤU TRÚC prompt (buildChatSystemPrompt/buildReconcileSystemPrompt)
// thay đổi đủ để có thể làm output khác đi — cache key (tokenEconomy.js/chat.js) PHẢI gồm giá trị
