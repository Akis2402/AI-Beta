...
const { callOpenAI, callOpenAIStream, isConfigured: openaiConfigured, MODEL: OPENAI_MODEL } = require('./openaiClient');
const { callGemini, callGeminiStream, isConfigured: geminiConfigured, MODEL: GEMINI_MODEL } = require('./geminiClient');
...
// Xây registry cho các provider "bổ sung" khai báo khai báo trong extraProviders.js — mỗi entry
// dùng chung 1 client tương thích OpenAI Chat Completions (createOpenAICompatibleClient), không
// hỗ trợ web search (đa số API tương thích OpenAI của bên thứ ba không có tool tìm kiếm tích hợp).
const EXTRA_REGISTRY = EXTRA_PROVIDERS.map((cfg) => {
...
const REGISTRY = [...CORE_REGISTRY, ...EXTRA_REGISTRY];

/**
 * Trả về danh sách provider đã có API key hợp lệ trong .env ngay tại thời điểm gọi (không cache),
 * để mọi thay đổi .env có hiệu lực ngay sau khi khởi động lại server mà không cần sửa code.
 */
...
module.exports = {
  getActiveProviders, callWithFailover, callFastest, streamWithFailover, shuffle, REGISTRY,
