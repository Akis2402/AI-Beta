...
// ---------- CIRCUIT BREAKER: né các provider vừa báo hết hạn mức (HTTP 429) trong ít phút ----------
// NGUYÊN NHÂN GỐC của việc "bấm Thử lại vẫn báo lỗi ngay lập tức như cũ, tốn cả phút mới biết":
// khi 1 provider trả về 429 kèm gợi ý rõ ràng ("Please try again in 48.2s"), các lượt gọi KẾ TIẾP
// (kể cả người dùng bấm "Thử lại" chỉ vài giây sau) vẫn cứ THỬ LẠI ĐÚNG provider đó trước tiên nếu
// random rơi trúng — tốn nguyên 1 lượt (tới ~20-30s) chờ timeout/lỗi dù CHẮC CHẮN sẽ lỗi (đã biết
// trước còn bao nhiêu giây mới hết hạn mức). FIX: ghi nhớ trong bộ nhớ tiến trình thời điểm 1
// provider hết hạn mức (đọc số giây "thử lại sau Ns" ngay trong thông báo lỗi của hãng nếu có, mặc
// định 60s nếu không đọc được) — trong lúc còn "nguội", mọi hàm điều phối (callWithFailover/
// callFastest/streamWithFailover/gatherCrossCheckCandidates) ĐẨY provider này xuống cuối thứ tự thử
// thay vì thử ngay, ưu tiên các provider chưa biết lỗi trước. KHÔNG loại bỏ hẳn — nếu mọi provider
// đều đang "nguội" cùng lúc, vẫn thử (còn hơn báo lỗi ngay mà chưa thử gì).
const providerCooldowns = new Map(); // providerKey -> timestamp (ms) hết cooldown

function markCooldown(providerKey, err) {
  if (!err || err.status !== 429) return;
  const text = String((err && (err.detail || err.message)) || '');
  const m = /try again in\s*([\d.]+)\s*s/i.exec(text);
  const retryMs = m ? Math.min(120000, Math.max(1000, Math.round(parseFloat(m[1]) * 1000))) : 60000;
  providerCooldowns.set(providerKey, Date.now() + retryMs);
}
function isCoolingDown(providerKey) {
  const until = providerCooldowns.get(providerKey);
  return !!until && until > Date.now();
}
/** Sắp lại thứ tự: provider đang "nguội" (429 gần đây, còn hạn) bị đẩy xuống cuối — không loại hẳn. */
function deprioritizeCoolingDown(order) {
  const fresh = order.filter((p) => !isCoolingDown(p.key));
  const cooling = order.filter((p) => isCoolingDown(p.key));
  return fresh.length ? [...fresh, ...cooling] : order;
}
...
async function gatherCrossCheckCandidates(providers, { system, variantSystem, messages, maxTokens, onStatus }) {
  ...
  // Provider đang "nguội" (429 gần đây, xem circuit breaker ở trên) bị đẩy xuống cuối nhóm gọi song
  // song — vẫn được gọi (không loại hẳn) nhưng không còn được ưu tiên ngang hàng provider "khỏe".
  const round1Order = deprioritizeCoolingDown(providers);
  const firstRound = await Promise.allSettled(
    round1Order.map((p) => p.call({ system: variantSystem, messages, maxTokens, timeoutMs: timeoutFor(30000) }))
  );
  ...
  firstRound.forEach((r, i) => {
    if (r.status === 'rejected') markCooldown(round1Order[i].key, r.reason);
  ...
  if (failedProviders.length && !deadline.expired()) {
    ...
    const retryOutcomes = await Promise.allSettled(
      failedProviders.map(async (failed) => {
        const others = deprioritizeCoolingDown(shuffle(providers.filter((p) => p.key !== failed.key)));
        ...
        for (const p of retryOrder) {
          ...
          } catch (e) { markCooldown(p.key, e); /* thử ứng viên kế tiếp trong retryOrder */ }
        }
...
// NGUYÊN NHÂN GỐC của "chờ rất lâu mới biết TẤT CẢ provider đều lỗi": trước đây mỗi provider được
// thử TUẦN TỰ với ĐẦY ĐỦ timeoutMs riêng (mặc định ~30s) — với 3 provider đã cấu hình, trường hợp
// xấu nhất (cả 3 đều lỗi/quá tải) người dùng phải chờ tới ~90s mới thấy thông báo lỗi, y hệt tình
// huống trong ảnh chụp màn hình (Gemini timeout 30s + Groq 429 + OpenRouter rỗng, cộng dồn tuần tự).
// FIX: áp dụng ngân sách thời gian TỔNG dùng chung cho toàn bộ vòng thử (giống cơ chế đã có ở
// gatherCrossCheckCandidates) — timeout của MỖI provider tự co lại theo ngân sách còn lại, và khi
// ngân sách gần hết (đã thử được ít nhất 1 provider), hệ thống NGỪNG thử thêm và báo lỗi ngay thay
// vì kéo dài vô thời hạn theo số provider đã cấu hình.
...
async function callWithFailover(providers, args, { preferWebSearch = false } = {}) {
  ...
  // Provider vừa báo hết hạn mức (429) gần đây bị đẩy xuống cuối — xem circuit breaker ở đầu file.
  order = deprioritizeCoolingDown(order);

  const deadline = createDeadline(FAILOVER_BUDGET_MS);
  const tried = [];
  let lastErr;
  for (const p of order) {
    if (deadline.expired() && tried.length) break; // đã thử ít nhất 1 provider và cạn ngân sách — dừng, báo lỗi ngay
    const callTimeout = Math.max(MIN_CALL_TIMEOUT_MS, Math.min(args.timeoutMs || 30000, deadline.remaining() || 30000));
    try {
...
async function callFastest(providers, args, { raceSize = 2 } = {}) {
  ...
  // Provider vừa báo hết hạn mức (429) gần đây bị đẩy xuống cuối — xem circuit breaker ở đầu file.
  const order = deprioritizeCoolingDown(shuffle(providers));
