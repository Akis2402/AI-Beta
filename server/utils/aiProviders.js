 ...
 */
async function gatherCrossCheckCandidates(providers, { system, variantSystem, messages, maxTokens, onStatus, deadline: parentDeadline, requestId }) {
  ...
  const firstRound = await Promise.allSettled(
    round1Order.map((p) => {
      ...
      return p.call({ system: variantSystem, messages, maxTokens, timeoutMs: t })
    ...
    const retryOutcomes = await Promise.allSettled(
      failedProviders.map(async (failed) => {
        ...
        for (const p of retryOrder) {
          ...
          const startedAt = Date.now();
          try {
            const text = stripThinkingTags(await p.call({ system: variantSystem, messages, maxTokens, timeoutMs: t }));
  ...
  if (candidates.length === 1 && !deadline.expired() && survivorTimeout !== null) {
    ...
    try {
      const extra = stripThinkingTags(await survivor.call({ system, messages, maxTokens, temperature: 0.4, timeoutMs: survivorTimeout }));
