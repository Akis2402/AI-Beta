 ...
 */
async function callClaude({ system, messages, maxTokens = 1000, tools, temperature, fast, timeoutMs = DEFAULT_TIMEOUT_MS, apiKeyOverride, modelOverride, fastModelOverride }) {
 ...
 */
async function callClaudeStream({ system, messages, maxTokens = 1000, tools, temperature, fast, timeoutMs = DEFAULT_TIMEOUT_MS, onDelta, apiKeyOverride, modelOverride, fastModelOverride }) {
  const key = apiKeyOverride || API_KEY;
  if (!key) {
    const err = new Error('Máy chủ chưa được cấu hình ANTHROPIC_API_KEY. Vui lòng liên hệ quản trị viên.');
    err.status = 500;
