 ...
 */
async function callGemini({ system, messages, maxTokens = 1000, temperature, webSearch, fast, timeoutMs = DEFAULT_TIMEOUT_MS }) {
  if (!API_KEY) {
  ...
  const modelId = fast ? MODEL_FAST : MODEL;
 ...
 * @returns {Promise<string>}
 */
async function callGeminiStream({ system, messages, maxTokens = 1000, temperature, webSearch, fast, timeoutMs = DEFAULT_TIMEOUT_MS, onDelta }) {
  if (!API_KEY) {
    const err = new Error('Máy chủ chưa cấu hình GEMINI_API_KEY.');
    err.status = 500;
    throw err;
  ...
  const modelId = fast ? MODEL_FAST : MODEL;
  const url = `https://generativelanguage.googleapis.com/v1beta/models/${modelId}:streamGenerateContent?alt=sse&key=${API_KEY}`;
  const body = {
    contents: toGeminiContents(messages),
    generationConfig: {
      maxOutputTokens: maxTokens,
