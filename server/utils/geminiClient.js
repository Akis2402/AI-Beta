 ...
 */
async function callGemini({ system, messages, maxTokens = 1000, temperature, webSearch, fast, deepThinking, capabilities, timeoutMs = DEFAULT_TIMEOUT_MS, apiKeyOverride, modelOverride, fastModelOverride, signal }) {
 ...
 * @returns {Promise<string>}
 */
async function callGeminiStream({ system, messages, maxTokens = 1000, temperature, webSearch, fast, deepThinking, capabilities, timeoutMs = DEFAULT_TIMEOUT_MS, onDelta, apiKeyOverride, modelOverride, fastModelOverride, signal }) {
  const key = apiKeyOverride || API_KEY;
  if (!key) {
    const err = new Error('Máy chủ chưa cấu hình GEMINI_API_KEY.');
    err.status = 500;
