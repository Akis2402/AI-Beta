 ...
 */
async function callGemini({ system, messages, maxTokens = 1000, temperature, webSearch, fast, timeoutMs = DEFAULT_TIMEOUT_MS, apiKeyOverride, modelOverride, fastModelOverride }) {
 ...
 * @returns {Promise<string>}
 */
async function callGeminiStream({ system, messages, maxTokens = 1000, temperature, webSearch, fast, timeoutMs = DEFAULT_TIMEOUT_MS, onDelta, apiKeyOverride, modelOverride, fastModelOverride }) {
  const key = apiKeyOverride || API_KEY;
  if (!key) {
    const err = new Error('Máy chủ chưa cấu hình GEMINI_API_KEY.');
    err.status = 500;
