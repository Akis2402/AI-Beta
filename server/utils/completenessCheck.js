 ...
 * @param {{stage?:'approach'|'detail', problemText?:string, coverageList?:string[]}} [opts]
 * @returns {{status:'COMPLETE'|'INCOMPLETE'|'INVALID', reasons:string[], missingCoverage:string[]}}
 */
function validateSolutionCompleteness(text, opts = {}) {
  const { stage = 'detail', problemText = '', coverageList } = opts;
  const clean = (text || '').trim();
  const reasons = [];

  if (clean.length < MIN_MEANINGFUL_LENGTH) {
    return { status: 'INVALID', reasons: ['response_empty_or_too_short'], missingCoverage: [] };
  }
  ...
  return { status, reasons, missingCoverage: missing };
}
