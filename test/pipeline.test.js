...
test('continuation: MAX_CONTINUATIONS is a small finite bound (no infinite retry)', () => {
  // FIX mục 3 (audit continuation): trần nâng từ 2 lên 6 để bài rất dài (5-6 ý phức tạp) không bị
  // chặt cụt sớm — vẫn phải là 1 số HỮU HẠN nhỏ (không phải hàng chục/vô hạn) để thực sự chặn được
  // vòng lặp khi provider cứ lặp lại lỗi y hệt (xem test/runtime.test.js mục 7).
  assert.ok(MAX_CONTINUATIONS >= 1 && MAX_CONTINUATIONS <= 8);
});
