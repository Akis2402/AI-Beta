...
// FIX ROOT CAUSE (PHẦN VII audit — mobile chậm do tải hết vendor lib ngay từ đầu): pdf.js/mammoth/
// three.js/docx.js giờ KHÔNG còn <script src> eager trong index.html — được app.js tự nạp on-demand
// qua window.__loadVendorScript() (định nghĩa trong boot.js) đúng lúc feature tương ứng được gọi
// (ensurePdfJs/ensureMammoth/ensureThree/ensureDocx). KaTeX + mathjs giữ eager (core, dùng ở hầu hết
// câu trả lời AI). pdf.worker.min.js vẫn không xuất hiện ở đâu bằng chuỗi tĩnh — pdf.js tự spawn
// Worker tới URL gán ở pdfjsLib.GlobalWorkerOptions.workerSrc.
const LAZY_LOADED_VENDOR = new Set([
  'vendor/pdfjs/pdf.min.js',
  'vendor/mammoth/mammoth.browser.min.js',
  'vendor/three/three.min.js',
  'vendor/docx/index.umd.js'
]);
const appJsForVendorRefs = fs.readFileSync(path.join(publicDir, 'js', 'app.js'), 'utf8');

console.log('\n== Mọi file vendor được tham chiếu (eager trong index.html HOẶC lazy on-demand trong app.js) phải tồn tại thật trên đĩa (không phải link chết) ==');
VENDOR_FILES.forEach((rel) => {
  ...
  // pdfjsLib.GlobalWorkerOptions.workerSrc (xem app.js), nên không cần xuất hiện ở đâu dạng chuỗi.
  if (rel === 'vendor/pdfjs/pdf.worker.min.js') return;
  if (LAZY_LOADED_VENDOR.has(rel)) {
    ok(appJsForVendorRefs.includes('/' + rel), `public/js/app.js có tham chiếu /${rel} (nạp lazy qua __loadVendorScript)`);
    ok(!html.includes('/' + rel), `index.html KHÔNG còn <script src> eager cho /${rel} (đã chuyển sang lazy-load)`);
  } else {
    ok(html.includes('/' + rel), `index.html có tham chiếu eager /${rel}`);
  }
});
