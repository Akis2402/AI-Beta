...
(async function main() {
  ...
  await test('vercel.json KHÔNG còn rewrite gộp /api/* về /api (giả định path không được bảo đảm)', () => {
    const vercel = JSON.parse(fs.readFileSync(path.join(__dirname, '..', 'vercel.json'), 'utf8'));
    assert.ok(!vercel.rewrites, 'định tuyến theo tệp (api/[...path].js) giữ nguyên req.url');
    assert.ok(fs.existsSync(path.join(__dirname, '..', 'api', '[...path].js')), 'phải có function catch-all cho subpath');
    assert.ok(fs.existsSync(path.join(__dirname, '..', 'api', 'index.js')), 'và function cho /api');
  });
