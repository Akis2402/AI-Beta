...
| **Rate limiting** | `server/middleware/security.js` | Giới hạn số request/15 phút theo IP — chống spam và giới hạn chi phí gọi Anthropic API (`RATE_LIMIT_CHAT`, `RATE_LIMIT_GENERATE`). **Lưu ý (audit P1/J):** dùng `express-rate-limit` với store mặc định = bộ nhớ trong tiến trình (`Map` in-memory) — đây là giới hạn **best-effort theo TỪNG instance**, KHÔNG phải quota chính xác toàn cục. Trên nền tảng serverless (Vercel), mỗi cold start/instance đếm riêng, nên tổng số request thực tế người dùng có thể gửi qua có thể VƯỢT con số cấu hình nếu Vercel spin nhiều instance song song. Muốn có quota chính xác toàn cục cần chuyển sang store dùng chung (Redis/Vercel KV) — dự án hiện KHÔNG làm việc này vì hướng tới deploy cá nhân/nhóm nhỏ, không phải multi-tenant công khai quy mô lớn. |
...
**Lỗi "Anthropic API trả về lỗi (HTTP 400)" (nếu gặp lại):** nguyên nhân phổ biến nhất là biến môi trường `ANTHROPIC_MODEL` (nếu bạn có đặt) trỏ tới một model string không hợp lệ/không còn tồn tại — Anthropic từ chối request ngay ở trường `model`. Bản hiện tại đã đổi model mặc định về `claude-sonnet-5`.
**FIX P2/O (audit — đính chính đoạn mô tả cũ không còn đúng):** đoạn README trước đây nói response lỗi hiển thị trên giao diện LUÔN kèm nội dung gốc từ Anthropic kể cả khi `NODE_ENV=production` — điều này KHÔNG còn đúng sau khi `server/utils/errorNormalize.js`/`anthropicClient.js` được siết lại để KHÔNG bao giờ lộ message thô của provider (có thể chứa chi tiết billing/nội bộ) ra client khi `NODE_ENV=production` (chỉ còn thông báo chung theo `code`, xem `GENERIC_MESSAGE_BY_CODE`). Hành vi THẬT hiện tại:
- **`NODE_ENV` khác `production`** (mặc định khi chạy `npm start`/`npm run dev` không set biến này): client thấy đầy đủ `err.message`/`detail` — bao gồm câu chữ gốc Anthropic trả về (vd `"model: xxx is not a valid model ID"`) — dễ tự chẩn đoán ngay khi phát triển local.
- **`NODE_ENV=production`** (khuyến nghị khi deploy công khai, kể cả trên Vercel): client CHỈ thấy thông báo chung an toàn (vd "Nhà cung cấp AI gặp lỗi khi trả lời. Vui lòng thử lại."), KHÔNG có chi tiết Anthropic gốc — tránh lộ dữ liệu nội bộ ra người dùng cuối. Muốn chẩn đoán lỗi 400 do model sai khi đã bật `NODE_ENV=production`, xem **log server** (Vercel → tab Logs của function, hoặc `console.error` khi chạy tự host) — `errorHandler.js` luôn ghi đầy đủ chi tiết lỗi (kèm provider/model) vào log bất kể `NODE_ENV`, chỉ response HTTP trả về client là bị giới hạn.

Nếu vẫn gặp 400 khi đang phát triển local (`NODE_ENV` chưa set `production`), đọc đúng câu chữ Anthropic trả về hiển thị trên giao diện — nó luôn nêu rõ trường nào trong request không hợp lệ.
